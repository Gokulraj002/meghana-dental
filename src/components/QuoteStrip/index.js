...
export default async function QuoteStrip() {
  const rows = await getQuotes();
  const quotes = rows.length > 0 ? rows : FALLBACK;
    ...
    <section style={{
      ...
      <div className="container">
        <div className="row g-4 justify-content-center">
          {quotes.map((q, i) => (
            <div className="col-lg col-md-4 col-sm-6" key={q.id || i}>
              <div style={{
                display: 'flex', flexDirection: 'column', alignItems: 'center',
                textAlign: 'center', gap: '12px', padding: '8px 12px',
              }}>
                <div style={{
                  ...
                  <i className={`bi ${q.icon || 'bi-quote'}`} style={{ fontSize: '1.3rem', color: '#fff' }} />
                </div>
                <p style={{ margin: 0, color: '#fff', fontSize: '0.95rem', fontWeight: '500', lineHeight: 1.5 }}>
                  {q.text}
                </p>
              </div>
            </div>
